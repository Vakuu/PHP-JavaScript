...
" . $myquery . "\r\n" ;
		...
		$fname_pref = $this->dbName."-".date("Y-m-dH-i-s");
		$fname = $fname_pref.".sql";
		    echo "\"".$GonxAdmin["mysqldump"]."\" --host ".$GonxAdmin["dbhost"]." --user=".$GonxAdmin["dbuser"]." --pass=".$GonxAdmin["dbpass"]." --databases ".$GonxAdmin["dbname"]." > ".$this->backupdir."/".$fname;
			@shell_exec("\"".$GonxAdmin["mysqldump"]."\" --host ".$GonxAdmin["dbhost"]." --user=".$GonxAdmin["dbuser"]." --pass=".$GonxAdmin["dbpass"]." --databases ".$GonxAdmin["dbname"]." > ".$this->backupdir."/".$fname);
				
	/*	switch($this->compression){
			case "bz2": 
			//$fname = $this->dbName."-".date("Y-m-dH-i-s").".bz2";
				$fname = $fname_pref.".bz2";
				touch($this->backupdir."/".$fname);
				$fp = bzopen($this->backupdir."/".$fname, "w");
				bzwrite($fp, $Dx_Create_Tables);
				bzclose($fp);
		    ...
		    case "zlib": 
				//$fname = $this->dbName."-".date("Y-m-dH-i-s").".gz";
				$fname = $fname_pref.".gz";
				touch($this->backupdir."/".$fname);
				$fp = gzopen($this->backupdir."/".$fname, "w");
				gzwrite($fp, $Dx_Create_Tables);
			...
			default:
				//$fname = $this->dbName."-".date("Y-m-dH-i-s").".sql";
				$fname = $fname_pref.".sql";
				touch($this->backupdir."/".$fname);
				$fp = fopen($this->backupdir."/".$fname, "w");
				fwrite($fp, $Dx_Create_Tables);
...
//copyright Pommef - IdЋes et Changements (c) 2004
...
\t<tr>
		...
		<p align=right>";
		/* за възстановяване на базата от архив.
		 да се пусне след внимателна преценка, защото има вероятност при неправилно
		изпълнение да се загубят данни
		if($this->mysqldump)
...
<table class=config cellspacing=0 cellpadding=4>
  <tr><td colspan=2><b>".$GONX["configureapp"]."</b><br>
  <span style=\"color:red\">
  Внимание: Промяната на стойностите, извършете много внимателно!<br>
  Ако е необходимо потърсете съдействие от разработчиците на системата
  </span><br>
  
  
  </td></tr>\n\n
  
  <tr align=center><th width=50%>
  <i>Променлива:</i></th><th width=50%><i>Стойност:</i></th></tr>\n\n";
...
<input type=hidden name=go value=saveconfig>
	...
	function saveconfig(){
		...
		// Assurons nous que le fichier est accessible en Ћcriture
		if (is_writable($filename)) {
		
		    // Dans notre exemple, nous ouvrons le fichier $filename en mode d'ajout
		    // Le pointeur de fichier est placЋ € la fin du fichier
		    // c'est l€ que $somecontent sera placЋ
		    if (!$handle = fopen($filename, 'w')) {
		         echo "Не могат да се запишат настройките ($filename)";
		         exit;
		    }
